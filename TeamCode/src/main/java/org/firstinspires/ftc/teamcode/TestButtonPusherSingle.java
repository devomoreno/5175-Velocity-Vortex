...
@TeleOp(name = "ButtonPusherSingle", group = "OpMode")
@Disabled

//import all hardware going to be used
public class TestButtonPusherSingle extends OpMode {
    ...
    double LoneServoPosition;



    public TestButtonPusherSingle(){}

    @Override
            public void init(){
        Treadleft=hardwareMap.dcMotor.get("LoneTread");
        Treadright=hardwareMap.dcMotor.get("RightTread");
        Treadleft.setDirection(DcMotor.Direction.REVERSE);
        LoneServo= hardwareMap.servo.get("LoneServo");
    ...
    @Override
        ...
        if(gamepad1.right_bumper){
            if(LoneServoPosition != 0){
                LoneServoPosition -= servoDelta;
            }
        ...
        if( gamepad1.left_trigger>0){
            if(LoneServoPosition != SERVO_MAX_RANGE){
                LoneServoPosition += servoDelta;
            }
        }

        LoneServoPosition = Range.clip(LoneServoPosition, SERVO_MIN_RANGE, SERVO_MAX_RANGE);

        // write position values to the wrist and claw servo
        LoneServo.setPosition(LoneServoPosition);
